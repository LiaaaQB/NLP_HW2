...
class BPETokenizer(BaseTokenizer):
    ...
    def train(self, texts: List[str]) -> None:
        ...
        # print(f'initialization completed at {end_time - start_time:.4f} seconds')
        # print(f'length of token_to_id: {len(self.token_to_id)}')
        ...
        print(f'Finished training in {total_time_e - total_time_s:.4f} seconds.')
    ...
    def find_bigrams(self, text):
        ...
        least_common = counter.most_common()[:-6:-1]  # last 5 entries

        print("Top 5 most common word bigrams:")
        for bigram, count in most_common:
            print(f"{bigram}: {count}")

        print("\nTop 5 least common word bigrams:")
        for bigram, count in least_common:
            print(f"{bigram}: {count}")
    ...
    def compare_(self):
            ...
            for id_val, token in self.id_to_token.items():
                if token not in self.token_to_id:
                    errors.append(f"Token '{token}' from id_to_token[{id_val}] not found in token_to_id.")
                elif self.token_to_id[token] != id_val:
                    errors.append(
                        f"Mismatch: id_to_token[{id_val}] = '{token}', but token_to_id['{token}'] = {self.token_to_id[token]}")

            for token, id_val in self.token_to_id.items():
                if id_val not in self.id_to_token:
                    errors.append(f"ID {id_val} from token_to_id['{token}'] not found in id_to_token.")
                elif self.id_to_token[id_val] != token:
                    errors.append(
                        f"Mismatch: token_to_id['{token}'] = {id_val}, but id_to_token[{id_val}] = '{self.id_to_token[id_val]}'")
            ...
            if errors:
                ...
                print("Dictionaries are correctly mapped in both directions.")
